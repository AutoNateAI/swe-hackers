...
class AudioNarrationEngine {
  ...
  async playStep(storyId, stepIndex) {
    return new Promise(async (resolve) => {
      let timestamps = this.getTimestamps(storyId, stepIndex);
      
      // Fallback: load timestamps from individual JSON file if not in manifest
      if (!timestamps) {
        try {
          const jsonPath = `${this.audioBasePath}/${this.currentVoice}/${storyId}/step-${stepIndex}.json`;
          const response = await fetch(jsonPath);
          if (response.ok) {
            timestamps = await response.json();
          }
        } catch (e) {
          console.warn('Could not load timestamps from file:', e);
        }
      }
